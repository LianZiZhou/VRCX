/**
 * [hub] What `vrcx-hub-migrate` does, independent of how it talks to the user.
 *
 * The four commands are written against a small `ui` contract so that the
 * same code drives both the terminal (`cli.js`) and the browser page
 * (`gui/server.js`): lines of text, a progress readout, and a yes/no question.
 * Neither front end holds any of the logic.
 */
...
import { HubSettingKey, resolveHubTarget } from './options.js';
import {
    BACKUPS_DIR,
    DATABASE_FILE,
    STORAGE_FILE,
...
export const TOOL_VERSION = typeof VERSION === 'undefined' ? readVersionFile() : VERSION;

/**
 * @typedef {object} CommandUi
 * @property {(text?: string) => void} say - one line of output
 * @property {(label: string) => (done: number, total: number) => void} progress - a progress readout
 * @property {(question: string, flags: Record<string, string | boolean>) => Promise<boolean>} confirm
 */
 ...
 * @returns {Source}
 */
export function locateSource(flags) {
    const dir = resolve(typeof flags.from === 'string' && flags.from ? flags.from : defaultDataDir());
    if (!existsSync(dir)) {
...
/**
 * @param {CommandUi} ui
 * @returns {{ migrate: Function, backup: Function, restore: Function, info: Function }}
 */
export function createCommands(ui) {
    const { say, progress, confirm } = ui;
    ...
    async function backup(flags) {
        if (given(flags.hub)) {
            const target = requireHubTarget(flags);
            const dest = resolve(given(flags.to) ? flags.to : `vrcx-backup-hub-${timestamp()}`);
        ...
        const dest = resolve(given(flags.to) ? flags.to : `vrcx-backup-${source.dataDir.kind}-${timestamp()}`);
        requireEmptyDir(dest);
    ...
    async function restore(flags) {
        ...
        const dest = resolve(given(flags.to) ? flags.to : defaultDataDir());
        ...
        if (digest !== manifest.database.sha256) {
            throw new Error(
                `${source.databasePath} does not match its manifest (digest ${digest}); the backup is damaged`
            );
        }
        printSummary(
    ...
    return { migrate, backup, restore, info };
}
